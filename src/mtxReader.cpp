...
void parseMtxStorage(MtxStructure &mtx, std::string storage) {
  ...
  if (storage == "coordinate") {
    ...
    throw std::runtime_error("Unsupported mtx storage '" + storage +
...
void parseMtxType(MtxStructure &mtx, std::string type) {
  ...
  if (type == "real") {
    ...
    throw std::runtime_error("Unsupported mtx value type '" + type +
...
COO<valueType> readCOO(std::ifstream &file, const MtxStructure &mtx) {
  ...
  for (size_t i = 0; i < mtx.num_entries; i++) {
    ...
    if (!ok) {
      throw std::runtime_error("Failed to read mtx entry at data line " +
                               std::to_string(i + 1));
    }

    if (row == 0 || row > mtx.num_rows || col == 0 || col > mtx.num_cols) {
      throw std::runtime_error("Out of bounds entry at data line " +
                               std::to_string(i + 1));
    }
...
std::ifstream openFile(const std::filesystem::path &path) {
  ...
  if (!file) {
    throw std::runtime_error("Failed to open Matrix Market file: " +
                             path.string());
  }
  return file;
}
